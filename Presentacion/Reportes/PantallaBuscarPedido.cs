...
{
    public class PantallaBuscarPedido : Pantalla
    {
        public static Factura Factura;
        public string Renglon;
        private DataTable _datos;
        private bool _retFactura;

        public PantallaBuscarPedido(bool factura = true)
        {
            InitializeComponent();
            ActivarFechas();
            Reiniciar();
        ...
        private void InitializeComponent()
        {
            ...
            this.dgvTabla.Columns.AddRange(new System.Windows.Forms.DataGridViewColumn[] {this.cid, this.cnombrecliente, this.cfecha, this.cimporte});
        ...
        private void BuscarPedido()
        {
            ...
            _datos = ManejadorReportes.CargarDatos("R_RecuperarVenta", parametros);
            LLenarTabla(_datos);
        ...
        private void LLenarTabla(DataTable datos)
        {
            ...
            {
                dgvTabla.RowCount++;
                dgvTabla["cid", dgvTabla.RowCount - 1].Value = dato["IDPedido"];
                dgvTabla["cnombrecliente", dgvTabla.RowCount - 1].Value = dato["Nombre"];
                dgvTabla["cimporte", dgvTabla.RowCount - 1].Value = dato["Importe"];
        ...
        private Dictionary<string, object> ConfigurarBusqueda()
        {
            ...
            DateTime fInicio = DateTime.MinValue;
            DateTime fFinal = DateTime.MaxValue;
            ...
            {
                opc = 1;
                parametros["idped"] = int.Parse(txtNumeroPedido.Text);
            }

            if (!string.IsNullOrWhiteSpace(txtNombreCliente.Text))
            {
                opc = opc == 1 ? 5 : 2;
                parametros["nmoc"] = txtNombreCliente.Text;
            }

            if (chkActivar.Checked)
            {
                fInicio = dtpFechaA.Value;
                fFinal = dtpFechaB.Value;
                ...
                parametros["fchi"] = fInicio;
                parametros["fchf"] = fFinal;
            }
        ...
        private void GenerarFactura(string idPedido)
        {
            DataRow ventaSeleccionada = _datos.Select($"IDPedido = {idPedido}").FirstOrDefault();
            ...
            {
                /*Informacion Cliente*/
                IdPedido = ventaSeleccionada["IDPedido"].ToString(),
                NombreC = $"{ventaSeleccionada["Nombre"]} {ventaSeleccionada["Apellidos"]}",
                CalleC = ventaSeleccionada["Direccion"].ToString(),
                ColoniaC = ventaSeleccionada["Colonia"].ToString(),
                CiudadC = ventaSeleccionada["Ciudad"].ToString(),
                EstadoC = ventaSeleccionada["Estado"].ToString(),
                PaisC = ventaSeleccionada["Pais"].ToString(),
                CpC = ventaSeleccionada["CodPos"].ToString(),
                TelefonoC = ventaSeleccionada["TelOficina"].ToString(),
                CorreoC = ventaSeleccionada["Email"].ToString(),
                RfcC = ventaSeleccionada["RFC"].ToString(),
                Importe = Convert.ToDecimal(ventaSeleccionada["Importe"]),
                /*Informacion Empresarial*/
                CiudadE = InformacionEmpresarial.Ciudad,
                CorreoE = InformacionEmpresarial.Correo,
                CpE = InformacionEmpresarial.CodigoPostal,
                CalleE = InformacionEmpresarial.Direccion,
                EstadoE = InformacionEmpresarial.Estado,
                TelefonoE = InformacionEmpresarial.Telefono,
                RfcE = InformacionEmpresarial.Rfc,
                NombreE = InformacionEmpresarial.Nombre,
                ColoniaE = InformacionEmpresarial.Colonia,
                PaisE = InformacionEmpresarial.Pais,
                NumCasaE = InformacionEmpresarial.NumeroCasa,
                /*Informacion factura*/
                Fecha = DateTime.Today
            };

            Factura.ListaProductos = new List<Productos>
            {
                new Productos(ventaSeleccionada["IDPedido"].ToString(), ventaSeleccionada["MatProd"].ToString(), ventaSeleccionada["Mat1"].ToString(),
                    Convert.ToDecimal(ventaSeleccionada["PrMat1"]), ventaSeleccionada["Mat2"].ToString(),
                    Convert.ToDecimal(ventaSeleccionada["PrMat2"]), Convert.ToDouble(ventaSeleccionada["Subtotal"]),
                    Convert.ToInt32(ventaSeleccionada["IDProducto"]))
            };
        }
        ...
        private void SeleccionarPedido()
        {
            ...
            {
                MessageBox.Show("No se han encontrado pedidos", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                MessageBox.Show("No se ha seleccionado un pedido.", "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if(_retFactura)
            {
                GenerarFactura(Renglon);
            }
            Hide();
        }
