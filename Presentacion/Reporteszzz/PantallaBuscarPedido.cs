...
namespace Presentacion.Reportes
{
    internal class PantallaBuscarPedido : Pantalla
    {
        ...
        private void BuscarPedido()
        {
            ...
            List<Resultados> pedidos = ConsultarPedido(new DatosBusqueda(fechaA, fechaB, numeroPedido, chkActivar.Checked, nombreCliente));

            dgvTabla.Rows.Clear();
            foreach (Resultados pedido in pedidos)
            {
                dgvTabla.RowCount++;
                dgvTabla["cid", dgvTabla.RowCount - 1].Value = pedido.Id;
                dgvTabla["cnombrecliente", dgvTabla.RowCount - 1].Value = pedido.NombreCliente;
                dgvTabla["cdescripcion", dgvTabla.RowCount - 1].Value = pedido.Descripcion;
        ...
        private void SeleccionarPedido(int facturaId)
        {
        ...
        {
            if (dgvTabla.SelectedRows.Count <= 0) return;
            if (dgvTabla.SelectedRows[0].Cells["cid"].Value != null)
                Renglon = (int) dgvTabla.SelectedRows[0].Cells["cid"].Value;
            ...
            if (Renglon > -1)
            {
                ...
                Validar.MensajeErrorOK("Sin pedidos");
        }
